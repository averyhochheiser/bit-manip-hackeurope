...
def load_config():
    ...
    if 'carbon-gate' not in config:
        output("Invalid carbon-gate.yml format. Missing 'carbon-gate' root key.", 'error')
        sys.exit(1)
...
def has_permission_to_override(username: str, config: dict) -> tuple[bool, str]:
    ...
    # Permission denied
    return False, f"@{username} has {user_permission} permissions (requires {required_permission})"
...
def extract_justification(text: str, command: str) -> Optional[str]:
    ...
    if justification and len(justification) >= 10:  # Minimum 10 chars for valid justification
    ...
    try:
        ...
        for event in reversed(events):
            if (event.get('event') == 'labeled' and 
                event.get('label', {}).get('name') == 'carbon-override'):
        ...
        if require_justification:
            ...
            for comment in comments:
                if comment.get('user', {}).get('login') == user:
                    ...
                    if any(keyword in body for keyword in ['override', 'exception', 'urgent', 'critical']):
...
def call_gate_api(config):
    ...
    output(f"Running Carbon Gate check for PR #{pr_number}", 'info', {
        'gpu': payload['gpu'],
...
def check_gate_status(config, result):
    ...
    if status == 'block':
        output(f"GATE BLOCKED: Emissions ({emissions:.2f} kg) exceed threshold ({threshold_kg} kg)", 'error', {
        ...
        output(f"WARNING: High carbon emissions ({emissions:.2f} kg)", 'warn', {
            'emissions_kg': emissions,
            'threshold_kg': threshold_kg,
            'status': 'warn'
        })
        output("Consider rerouting to Crusoe or waiting for optimal grid window", 'info')
    else:
        output(f"GATE PASSED: Emissions ({emissions:.2f} kg) within acceptable limits", 'success', {
            'emissions_kg': emissions,
            'threshold_kg': threshold_kg,
            'status': 'pass'
        })
