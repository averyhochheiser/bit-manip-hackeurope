...
export function OverageBillingCard({ includedKg, usedKg, unitPrice }: OverageBillingCardProps) {
  ...
  return (
    <section className="panel p-5">
      <h3 className="text-base font-semibold text-floral">Overage Billing</h3>
      <div className="mt-4 space-y-2 text-sm text-floral/75">
      ...
      <div className="mt-4 rounded-xl border border-floral/10 bg-floral/[0.03] p-3">
        <p className="text-xs uppercase tracking-[0.16em] text-floral/50">Estimated Stripe charge</p>
        <p className="mt-1 font-monoData text-2xl font-semibold text-floral">${estimate.toFixed(2)}</p>
      </div>
    </section>
  );
}
