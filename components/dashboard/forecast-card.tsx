  ...
  return (
    <section className="panel p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-xs text-white/60">Fourier-smoothed</span>
      </div>
      <div className="flex h-24 items-end gap-2">
        {MOCK_FORECAST.map((point, index) => {
          const height = Math.max(10, (point / max) * 100);
          return (
            <div key={`${point}-${index}`} className="relative flex-1 rounded-md bg-white/5">
              <div
                className="absolute bottom-0 w-full rounded-md bg-gradient-to-t from-crusoe/75 to-emerald-300/75"
                style={{ height: `${height}%` }}
              />
            </div>
          );
